				...
				<div className="sidebar-item p-4 cursor-pointer" onClick={()=>onClickHandler()}>
					<div className='flex items-center mb-1'>
						<div className='relative w-8 h-8 flex justify-center items-center'>
							<BsSunFill size={24} className={theme === 'dark' ? 'active toggle-theme text-default-text mr-2' : 'inactive toggle-theme text-default-text mr-2'}/>
							<BsFillMoonStarsFill size={24} className={theme === 'light' ? 'active toggle-theme text-default-text mr-2' : 'inactive toggle-theme text-default-text mr-2 '}/>
						</div>
						<span className='text-default-text text-sm'>Change Theme</span>
					</div>
					<small className='text-default-muted'>Current Theme : {theme}</small>
				</div>
			</div>
