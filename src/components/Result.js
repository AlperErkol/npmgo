      ...
      <div className="pt-20 max-w-screen-xl mx-auto overflow-hidden">
        {!error &&
          ...
          data.map((repositories) => (
            <Link
              key={repositories.package.name}
              className="max-w-screen-sm block"
              to={`/package/${repositories.package.name}`}
              state={{ packageDetails: repositories }}
            >
              <div className="group relative mb-8 last:mb-0 py-4 flex justify-between flex-col cursor-pointer w-full">
                <div className="flex items-center">
                  ...
                  <span className="text-sm text-default-text">
                    {repositories.package.publisher.username}
                  </span>
                </div>
                <div>
                  <div className="flex items-center">
                    <p className="inline font-semibold text-lg text-default-whitely group-hover:underline group-hover:underline-offset-4 mr-4 mb-0">
                      {repositories.package.name} |{" "}
                      {repositories.package.version}
                    </p>
                    {repositories.searchScore >= 10000 ? (
                      <span className="text-sm font-semibold text-default-text">
                        exact match
                      </span>
                    ) : null}
                  </div>
                  ...
                  <ul className="keywords mt-2">
                    {repositories.package.keywords &&
                      repositories.package.keywords.map((keyword) => (
                        <li
                          key={keyword}
                          className="text-sm inline-block py-1 px-2 mr-2 mb-2 bg-default-tertiary text-default-whitely rounded"
                        >
                          {keyword}
                        </li>
                      ))}
        ...
        {!error && !loading && data.length < 1 ? (
          <div className="w-full flex flex-col items-center justify-center mt-4">
            <div className="p-4 bg-default-sidebarColor text-center">
              <p className="text-default-text text-xl">
                No result for{" "}
                <span className="text-default-tertiary">{params.term}</span>.
              </p>
              <div>
